...
type ModalInProps = {
  ...
  inOrder: string;
  setInOrder: (v: string) => void;
  onConfirm: () => void;
  onClose: () => void;
};

export function ModalIn({ rawMat, inRawId, setInRawId, inQty, setInQty, inOrder, setInOrder, onConfirm, onClose }: ModalInProps) {
  ...
  return (
    <Modal title="Приход материала" icon="ArrowDownToLine" iconColor="#16a34a" onClose={onClose}>
      ...
      <label className="block mt-3 mb-1 text-[12px] text-[#6b6b6b]">ID заказа (необязательно)</label>
      <input value={inOrder} onChange={e => setInOrder(e.target.value)}
