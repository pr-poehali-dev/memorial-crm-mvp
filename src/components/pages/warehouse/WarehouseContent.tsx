...
/* ── Проблемные материалы (сырьё) ── */
function ProblemRawBlock({
  rawMat, getReserved,
}: { rawMat: RawMaterial[]; getReserved: (id: string) => number }) {
  ...
  return (
    <div className="bg-amber-50 border border-amber-200 rounded-xl overflow-hidden">
      <button
        onClick={() => setOpen(v => !v)}
        className="w-full flex items-center gap-2 px-4 py-3 hover:bg-amber-100/50 transition-colors"
      >
...
function ProblemBlanksBlock({
  blanks, getBlankReserved,
}: { blanks: Blank[]; getBlankReserved: (id: string) => number }) {
  const [open, setOpen] = useState(false);
...
export default function WarehouseContent({
  ...
  return (
    <>
      ...
      {tab === "raw" && (
        <>
          <ProblemRawBlock rawMat={rawMat} getReserved={getReserved} />
          <RawTable
      ...
      {tab === "blanks" && (
        <>
          <ProblemBlanksBlock blanks={blanks} getBlankReserved={getBlankReserved} />
          <BlanksTable
